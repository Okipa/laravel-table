                ...
                <tbody>
                    ...
                    @forelse($rows as $model)
                        <tr wire:key="row-{{ $model->getKey() }}"{!! $orderColumn ? ' wire:sortable.item="' . $model->getKey() . '"' : null !!} @class(array_merge(Arr::get($tableRowClass, $model->getKey(), []), ['border-bottom']))>
                            ...
                            @foreach($columns as $column)
                                @if($loop->first)
                                    <th wire:key="cell-{{ $column->getAttribute() }}-{{ $model->getKey() }}"{!! $orderColumn ? ' wire:sortable.handle' : null !!} class="align-middle" scope="row">{!! $orderColumn ? '<span class="me-2">' . config('laravel-table.icon.drag_drop') . '</span>' : null !!}{{ $column->getValue($model, $tableColumnActionsArray) }}</th>
                                @else
